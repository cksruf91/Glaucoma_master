...
with tf.device('/device:GPU:0'):
    ...
    # learning_rate = tf.train.exponential_decay(opts.LEARNING_RATE, global_step,
    #                                            opts.LR_DECAY_STEPS, opts.LR_DECAY_RATE)
...
with tf.variable_scope('optimizers'):
    ...
    # optimizer = tf.train.MomentumOptimizer(learning_rate=opts.LEARNING_RATE,
    #                                        momentum=opts.MOMENTUM, use_nesterov=True)
...
with tf.Session() as sess:
    ...
    print('train data from : {TRAIN_FILE}'.format(TRAIN_FILE=TRAIN_FILE))
    ...
    print('test data from : {TEST_FILE}'.format(TEST_FILE=TEST_FILE))
    ...
    for epoch_ in range(EPOCHS - last_epoch):
        ...
        print(' Current Learning Rate : {lr}'.format(lr=lr))
        ...
        print('\t- val loss : {mean_val_loss:5.5f}\t- val acc : {mean_val_acc:5.5f}'.format(mean_val_loss=mean_val_loss,
        ...
        print('time: ', epoch_time,
              '\ttrain auc : {auc:5.5f},\tvalidation auc : {val_auc:5.5f}'.format(
        ...
        if SEND_MESSAGE:
            message = 'epoch : {} | time : {} \n matrix : {:5.5f}'.format(epoch_, epoch_time,
                                                                          confusion_matrix(y_pred > 0.5, y_target))
            slack_message('#resnet_project', message)

        # save checkpoint
        if SAVE_CHECKPOINT:
            save_path = saver.save(sess, os.path.join(RESULT_PATH, 'checkpoint_{epoch}.ckpt'.format(epoch=epoch_)))

        # save history
        history_line = '{epoch},{mean_train_loss},{auc},{mean_val_loss},{val_auc}\n'.format(
            epoch=epoch_, mean_train_loss=mean_train_loss, auc=auc, mean_val_loss=mean_val_loss, val_auc=val_auc
        )
        if epoch_ == 1:
